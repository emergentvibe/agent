...
async function runScenario(scenarioName: string): Promise<AssertionResult[]> {
  ...
  if (process.env.DUMP_SEED === '1') {
    try {
      ...
      const topN = parseInt(process.env.SCHEDULE_TOP_N || '3', 10);
      const floorScore = parseFloat(process.env.SCHEDULE_FLOOR_SCORE || '0.15');
      ...
      for (const query of queries) {
        ...
        for (const m of results) {
          ...
          if (m.score !== undefined && m.score < floorScore) continue;
          seenIds.add(m.id);
          allResults.push(m);
          taken++;
        }
