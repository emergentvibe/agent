...
const hasFiles =
  fs.existsSync(ATTENDEE_PATH) && fs.existsSync(ROTA_PATH);

describe.skipIf(!hasFiles)(
  'sheet-adapter integration (sample data)',
  () => {
    let attendeeRaw: any;
    ...
    describe('attendee import', () => {
      ...
      it('handles Nana unicode display name', () => {
        ...
        attendeeImport(payload);
        const nana = attendeeRaw.people.find(
          (p: any) => p.name === 'Nana',
        );
    ...
    describe('rota import', () => {
      ...
      it('imports all 133 assignments when is_test overridden', () => {
        const overridden = {
          ...rotaRaw,
          is_test: false,
          generated_at: '2026-09-08T10:34:00+02:00',
        };
        const payload = adaptRotaExport(overridden);
      ...
      it('stores no_shifts entries', () => {
        ...
        const reason = rotaGetNoShiftReason(
          rotaRaw.no_shifts[0].person_id,
        );
