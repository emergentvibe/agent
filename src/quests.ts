...
export function questGetOptedIn(): Array<{
  ...
  const db = _getDb();
  return db.prepare('SELECT telegram_id, attendee_name FROM quest_optins').all() as Array<{
    telegram_id: string;
    attendee_name: string;
  }>;
}
...
export function questLogDelivery(
  telegramId: string,
  questText: string,
): void {
  const db = _getDb();
...
export async function deliverQuests(opts: QuestLoopOpts): Promise<number> {
  ...
  for (const user of selected) {
    try {
      ...
      logger.warn(
        { err, userId: user.telegram_id },
        'Failed to deliver quest',
      );
    }
  }
...
const QUEST_INTERVAL = 3 * 60 * 60 * 1000; // 3 hours
const QUIET_START = 22;
const QUIET_END = 9;

export function startQuestLoop(opts: QuestLoopOpts): void {
  logger.info({ intervalMs: QUEST_INTERVAL }, 'Quest delivery loop started');

  setInterval(async () => {
    const hour = new Date().getHours();
    if (hour < QUIET_END || hour >= QUIET_START) {
      logger.debug('Quest delivery skipped (quiet hours)');
      return;
    }
    try {
...
export function registerQuestCommands(
  bot: Bot,
  opts: QuestCommandOpts,
): void {
  bot.command('quests', async (ctx) => {
    ...
    if (questIsOptedIn(telegramId)) {
      ...
      await ctx.reply(
        'Opted out of quests. You can /quests again to rejoin.',
      );
    } else {
  ...
  bot.command('quest', async (ctx) => {
    ...
    if (ctx.chat.type !== 'private') {
      await ctx.reply('Send /quest to me in a DM!');
      return;
    }
    ...
    if (!questIsOptedIn(telegramId)) {
      await ctx.reply(
        "You're not signed up for quests yet. Send /quests to join!",
      );
      return;
    }
